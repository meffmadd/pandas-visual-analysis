...
class BrushSummaryWidget(BaseWidget):
    ...
    def __init__(self, data_source: DataSource, row: int, index: int, relative_size: float, max_height: int):
        ...
        self.base_metrics = self.data_source.data[self.columns] \
    ...
    def _update_brushed_metrics(self):
        ...
        with self.grid.hold_trait_notifications():
            self.grid[0, 2].value = self._get_metric_html_content(brush_count, brush_count / self.data_source.len - 1)
            ...
            for i, col in enumerate(self.columns):
                ...
                self.grid[(i + 1), 2].value = self._get_metric_html_content(metric_value, diff)
    ...
    def _update_base_metrics(self):
        ...
        with self.grid.hold_trait_notifications():
            self.grid[0, 1].value = self._get_metric_html_content(self.data_source.len, None)
            self.grid[0, 3].value = ""

            for i, col in enumerate(self.columns):
                self.grid[(i + 1), 1].value = self._get_metric_html_content(self.base_metrics[col][metric])
    ...
    def _get_indicator_html_content(self, diff: float) -> str:
        ...
        content = '<svg xmlns="http://www.w3.org/2000/svg" width="%d" height="%d" viewBox="0 0 24 24" ' \
    ...
    def _get_brushed_metrics(self):
        return self.data_source.brushed_data[self.columns] \
            .describe(include='all')
