...
class BoxPlotWidget(BaseWidget):
    ...
    def __init__(self, data_source: DataSource, row: int, index: int, relative_size: float, max_height: int):
        ...
        if len(self.columns) < 1:
            raise ValueError("The data contains too few numerical columns to display box plots. "
                             "Remove the widget from the layout!")

        self.column_select = widgets.Dropdown(options=self.columns, value=self.columns[0], description="Column:",
                                              style={"description_width": "60px"},)
        self.box_point_select = widgets.Dropdown(options=[("All", "all"), ("Outliers", "outliers"), ("None", False)],
                                                 value="all", description="Points:",
    ...
    def build(self) -> widgets.Widget:
        root = widgets.VBox([widgets.HBox([self.column_select, self.box_point_select]),
                             self.figure_widget])
    ...
    def _get_figure_widget(self):
        config = Config()
        trace = go.Box(y=self.data_source.data[self.column_select.value],
                       boxmean='sd',
    ...
    def _on_column_change(self, change):
        self.figure_widget.data[0].update({"y": self.data_source.data[self.column_select.value]})
