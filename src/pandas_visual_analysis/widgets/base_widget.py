...
class BaseWidget(HasTraits):
    ...
    def build(self) -> widgets.Widget:
        """This method returns an IPython Widget root node containing all the
        children for this widget."""
        pass

    def apply_size_constraints(self, widget):
        """Adds styling to a widget to control the height, width, margin,
        padding and border. Sets min and max_width to conform to the relative
        size of the widget, minus the margin. Sets min and max_width to conform
        to the max_height parameter. Adds a margin to the widget and also a
    ...
    def observe_brush_indices_change(self, change):
        """This method observes the changes in the brush selection. In order to
        actually observe changes it has to be registered in
        :meth:`set_observers`.
    ...
    def on_selection(self, trace, points, state):
        """This method implements the behaviour of changes in the selection of
        this plot. Should set the brushed indices property of
        :class:`pandas_visual_analysis.data_source.DataSource` in order to
    ...
    def on_deselection(self, trace, points):
        """This method implements the behaviour of changes in the deselection
        of this plot. Should reset the brushed selection of
        :class:`pandas_visual_analysis.data_source.DataSource` in order to
        propagate the change.
