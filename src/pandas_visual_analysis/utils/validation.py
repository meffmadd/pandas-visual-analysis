...
def validate_data(data, name="data"):
    if not isinstance(data, (DataFrame, pandas_visual_analysis.DataSource)):
        raise TypeError("The %s parameter must be a Pandas DataFrame or a DataSource" % name)
...
def validate_row_height(row_height, name="row_height"):
    ...
    if row_height < 0:
        raise ValueError("The value for %s has to be larger than 0. Invalid Value: %d" % (name, row_height))
...
def validate_sample(sample, name="sample"):
    if not (isinstance(sample, float) or isinstance(sample, int) or sample is None):
        raise TypeError("The value of the parameter %s has to be a floating point value, not %s" %
...
def validate_color(color):
    ...
    if not (0 <= color[0] <= 255) \
        ...
        raise ValueError("RGB values have to be between 0 and 255. Invalid values: %s" % str(color))
